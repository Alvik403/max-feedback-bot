...
def _format_dt_ru(created_at: str | None) -> str:
    ...
    try:
        ...
        return (
            f"{dt.day} {_MONTHS_GEN[dt.month - 1]} {dt.year}, "
...
def _format_submissions_list(rows: list) -> str:
    if not rows:
        return (
            "Мои обращения\n\n"
            "Здесь появятся ваши заявки из бота. Пока список пуст.\n\n"
            "Вернитесь в настройки кнопкой ниже."
        )
    blocks: list[str] = [
        "Мои обращения",
        "",
    ...
    for i, r in enumerate(rows, 1):
        kind_ru = (
            "Предложение" if r.get("kind") == "proposal" else "Обращение / жалоба"
        )
        vis = "анонимно" if r.get("anonymous") else "публично"
        dt = _format_dt_ru(r.get("created_at"))
        raw_txt = (r.get("text") or "").strip()
        preview = raw_txt[:500] + ("…" if len(raw_txt) > 500 else "")
        meta = f"{dt} · {vis}"
        # Карточка: номер + тип, метаданные отдельной строкой, текст с отступом
        blocks.append(f"{i}. {kind_ru}")
        blocks.append(f"   {meta}")
        blocks.append("")
        for ln in preview.split("\n"):
            blocks.append(f"   {ln}")
        blocks.append("")
    blocks.append("В настройки — кнопкой ниже.")
    return "\n".join(blocks).rstrip()
...
def settings_keyboard() -> str:
    return ButtonsPayload(
        buttons=[
            ...
            [CallbackButton(text="Мои обращения", payload="menu:my_submissions")],
...
async def on_callback(event: MessageCallback, context: MemoryContext) -> None:
    ...
    if pl.startswith("ticket:reply:"):
        ...
        await context.update_data(reply_ticket_id=tid, draft_prompt_mid=None)
    ...
    if pl == "input:cancel":
        if st == Flow.awaiting_ticket_reply:
            ...
            await _try_delete_prompt_message(event.message, b)
            await context.set_state(Flow.main)
            await context.update_data(reply_ticket_id=None, draft_prompt_mid=None)
            await b.send_message(
                **kw,
    ...
    if pl == "menu:my_submissions" and st == Flow.settings:
        await ack()
        await _try_delete_prompt_message(event.message, b)
        rows = await db_run(storage.list_user_submissions, uid, 25)
        msg = _format_submissions_list(rows)
        max_len = 3500
        if len(msg) <= max_len:
            await b.send_message(**kw, text=msg, attachments=[settings_keyboard()])
        else:
            chunk = msg[:max_len]
            await b.send_message(
                **kw,
                text=chunk + "\n\n… (часть списка не поместилась)",
                attachments=[settings_keyboard()],
            )
        return
...
async def on_ticket_user_reply(event: MessageCreated, context: MemoryContext) -> None:
    ...
    await event.message.answer(
        _plain_then_main_menu_html("Ответ отправлен.\n\n", row_ok),
        parse_mode=ParseMode.HTML,
        attachments=[main_menu_keyboard()],
    )
